...
function fmtDate(s) { if (!s) return "—"; return new Date(s).toLocaleDateString("en-AU", { day: "numeric", month: "short", year: "numeric" }); }
...
function ProfileTab({ child, rooms, onSaved }) {
  ...
  const save = async () => {
    setSaving(true);
    await API(`/api/children/${child.id}`, { method: "PUT", body: f });
    setSaving(false); setEd(false); onSaved(child.id);
  };
...
function AuthCollectionList({ childId, editable }) {
  ...
  const add = async () => {
    const r = await API(`/api/children/${childId}/collection-persons`, { method: "POST", body: nf });
...
function FocusTab({ child }) {
  ...
  const addNote = async () => {
    ...
    const r = await API(`/api/children/${child.id}/educator-notes`, { method: "POST", body: { note: newNote } });
...
function MedicalPlansSection({ child }) {
  ...
  const add = async (f) => {
    const r = await API(`/api/children/${child.id}/medical-plans`, { method: "POST", body: f });
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 8, alignItems: "end" }}>
      ...
      <div>
        ...
        <input type="date" style={inp} value={f.expiry_date} onChange={e => setF(p => ({ ...p, expiry_date: e.target.value }))} />
      </div>
      <div>
        <label style={lbl}>Notes</label>
...
function MedicationForm({ childId, onSaved, onClose }) {
  ...
  const save = async () => {
    const r = await API(`/api/children/${childId}/medications`, { method: "POST", body: f });
    if (r.id) onSaved();
  };
  return (
    <div style={{ marginTop: 12, background: lp, borderRadius: 10, padding: 14 }}>
...
function EquipmentForm({ childId, onSaved, onClose }) {
  ...
  const save = async () => {
    const r = await API(`/api/children/${childId}/equipment`, { method: "POST", body: f });
    if (r.id) onSaved();
  };
  return (
    <div style={{ marginTop: 12, background: lp, borderRadius: 10, padding: 14 }}>
...
function DietaryForm({ childId, onSaved, onClose }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    const r = await API(`/api/children/${childId}/dietary`, { method: "POST", body: f });
    setSaving(false);
    if (r.id) onSaved();
  };
  return (
            ...
            <div style={{ gridColumn: "span 3", background: "#FFEBEE", borderRadius: 8, padding: "12px 14px", border: "1px solid #FFCDD2" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8 }}>
                ...
                <div>
                  ...
                  <input type="date" style={inp} value={f.risk_minimisation_plan_date} onChange={e => setF(p => ({ ...p, risk_minimisation_plan_date: e.target.value }))} />
                </div>
                ...
                <div>
                  ...
                  <input type="date" style={inp} value={f.medical_communication_plan_date} onChange={e => setF(p => ({ ...p, medical_communication_plan_date: e.target.value }))} />
                </div>
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
                  <label style={{ fontSize: 10, color: "#2E7D32", fontWeight: 700 }}>
...
function ParentalRequestForm({ childId, onSaved, onClose }) {
  ...
  const save = async () => {
    const r = await API(`/api/children/${childId}/parental-requests`, { method: "POST", body: f });
    if (r.id) onSaved();
  };
  return (
    <div style={{ marginTop: 12, background: lp, borderRadius: 12, padding: 14 }}>
...
function ImmunisationTab({ child }) {
  ...
  const addRecord = async (f) => {
    const r = await API(`/api/children/${child.id}/immunisations`, { method: "POST", body: f });
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 8 }}>
        ...
        <div><label style={lbl}>Given Date</label><input type="date" style={inp} value={f.given_date} onChange={e => setF(p => ({ ...p, given_date: e.target.value }))} /></div>
        <div><label style={lbl}>Due Date</label><input type="date" style={inp} value={f.due_date} onChange={e => setF(p => ({ ...p, due_date: e.target.value }))} /></div>
...
function AddChildModal({ rooms, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    await API("/api/children", { method: "POST", body: f });
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: 520, maxWidth: "90vw" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          ...
          <div><label style={lbl}>Date of Birth</label><input type="date" style={inp} value={f.dob} onChange={e => setF(p => ({ ...p, dob: e.target.value }))} /></div>
...
function EducatorNotesTab({ child }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await API("/api/register/educator-notes", { method: "POST", body: { ...f, child_id: child.id } });
      ...
      <div style={{ ...card }}>
        ...
        {showAdd && (
          <div style={{ background: lp, borderRadius: 12, padding: 14, marginBottom: 14 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 8 }}>
              <div><label style={lbl}>Date</label><input type="date" style={inp} value={f.note_date} onChange={e => setF(p => ({ ...p, note_date: e.target.value }))} /></div>
              <div><label style={lbl}>Category</label>
                <select style={inp} value={f.category} onChange={e => setF(p => ({ ...p, category: e.target.value }))}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                      <span style={{ fontWeight: 700, fontSize: 11, color: "#3D3248" }}>
                        {new Date(note.note_date + "T12:00:00").toLocaleDateString("en-AU", { weekday: "short", day: "numeric", month: "short", year: "numeric" })}
                      </span>
...
function ParentMessagingTab({ child }) {
  ...
  const send = async () => {
    ...
    setSending(true);
    try {
      await API("/api/register/parent-messages", { method: "POST", body: { ...f, child_id: child.id } });
                ...
                <div key={msg.id} style={{ background: "#FDFBF9", borderRadius: 10, padding: "12px 14px", border: "1px solid #EDE8F4", borderLeft: `3px solid ${mt.color}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 10, color: "#8A7F96", whiteSpace: "nowrap" }}>
                      {new Date(msg.created_at).toLocaleDateString("en-AU", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                    </span>
                  </div>
