...
export function ComplianceDashboard({ rooms }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setComp(c||{ items: [], summary: {} }); setAtt(a||null); setNotifs(Array.isArray(n)?n:[]);
...
function NotificationsTab({ notifs, onRefresh }) {
  const mark = async id => { try { await API(`/api/compliance/notifications/${id}/send`,{method:"PUT"}); onRefresh(); } catch(e) { toast("Failed to send notification.", "error"); } };
  return (<div>
...
function ChildCompliance({ child, onBack }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPlans(Array.isArray(p)?p:[]); setMeds(Array.isArray(m)?m:[]); setParents(Array.isArray(pa)?pa:[]);
    } catch(e) { console.error('Compliance child load:', e); }
    setLoading(false);
  }, [child.id]);
  useEffect(() => { load(); }, [load]);
