        ...
        <div style={{ ...card }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <div>
              ...
              <DatePicker value={form.excursion_date||""} onChange={v => setForm({ ...form, excursion_date: v })} />
            </div>
            <div>
...
function ExcursionDetail({ excursion, children, educators, onToggleChild, onToggleEducator, onSendPermissions, onUpdateStatus, onRefresh }) {
  ...
  const handlePrintSlips = () => {
    const children = excursion.children || [];
    const ex = excursion;
    const slipHtml = children.map(ch => {
      return '<div style="border:1px solid #ccc;border-radius:8px;padding:16px 20px;margin-bottom:16px;page-break-inside:avoid">' +
        '<div style="display:flex;justify-content:space-between;margin-bottom:12px">' +
        '<div><strong style="font-size:16px">' + (ch.first_name||ch.name||'') + ' ' + (ch.last_name||'') + '</strong></div>' +
        '<div style="color:#666;font-size:13px">' + ex.title + '</div></div>' +
        '<p style="font-size:13px;margin:0 0 8px"><strong>Date:</strong> ' + ex.excursion_date + '</p>' +
        '<p style="font-size:13px;margin:0 0 8px"><strong>Destination:</strong> ' + ex.destination + '</p>' +
        '<p style="font-size:13px;margin:0 0 8px"><strong>Departure:</strong> ' + ex.departure_time + ' &nbsp; <strong>Return:</strong> ' + ex.return_time + '</p>' +
        '<p style="font-size:13px;margin:0 0 16px"><strong>Transport:</strong> ' + ex.transport_method + '</p>' +
        '<div style="margin-top:16px;padding-top:12px;border-top:1px solid #eee">' +
        '<p style="font-size:12px;margin:0 0 8px">I give permission for my child to attend this excursion.</p>' +
        '<div style="display:flex;gap:40px;margin-top:12px"><div>Signature: ___________________________</div><div>Date: _______________</div></div></div></div>';
    }).join('');
    const w = window.open('', '_blank');
    w.document.write('<html><head><title>Permission Slips</title>' +
      '<style>body{font-family:Arial,sans-serif;padding:24px;max-width:700px;margin:0 auto}@media print{.noprint{display:none}}</style>' +
      '</head><body><div class="noprint" style="margin-bottom:20px">' +
      '<button onclick="window.print()" style="padding:8px 20px;background:#3D3248;color:#fff;border:none;border-radius:6px;cursor:pointer">Print All Slips</button>' +
      '</div><h2>' + ex.title + ' — Permission Slips</h2>' +
      '<p style="color:#666">' + ex.excursion_date + ' · ' + ex.destination + '</p>' +
      slipHtml + '</body></html>');
    w.document.close();
  };
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexDirection: "column", alignItems: "flex-end" }}>
            ...
            {excursion.status === "permission_sent" && (
              <>
                <button onClick={handlePrintSlips}
                  style={{ background: "#5B8DB5", color: "#fff", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                  ...
                  return (
                    <tr key={ec.id || ec.child_id} style={{ borderBottom: "1px solid #F0EBF8" }}>
                      ...
                      <td style={{ padding: "10px 12px", color: "#8A7F96" }}>
                        {ec.permission_granted_at ? new Date(ec.permission_granted_at).toLocaleDateString(undefined) : "—"}
                      </td>
                    </tr>
                  );
                })}
