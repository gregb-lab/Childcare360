  ...
  const save = async (publish=false) => {
    ...
    try {
      ...
      if (editMode && editStoryData) {
        const sr = await API(`/api/learning/stories/${editStoryData.id}`, {method:"PUT", body:payload});
        if(sr.error){setSaveErr(sr.error);setSaving(false);return;}
        id = editStoryData.id;
      } else {
        ...
        <div>
          ...
          <div style={card}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:14}}>
              <div><label style={lbl}>Date</label><DatePicker value={f.date||""} onChange={v=>u("date",v)} /></div>
              <div>
...
function StoryDetailView({ story: s, children, onClose, onEdit, onRefresh }) {
  ...
  const recordProg = async () => {
    ...
    } catch(e) { toast("Failed to save progression.", "error"); return; }
...
function AlbumsView({ albums, children, stories, onOpen, onRefresh }) {
  ...
  const create = async () => {
    if(!name.trim()) return;
    try {
      await API("/api/learning/albums",{method:"POST",body:{name:name.trim()}});
