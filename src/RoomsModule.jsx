...
const API = async (path, opts = {}) => {
  ...
  if (!res.ok || data?.error) {
    ...
    console.error(`[RoomsModule] ${opts.method||'GET'} ${path} → ${msg}`);
    throw new Error(msg);
  }
  return data;
};
...
const DEFAULT_AGE_GROUPS = [
  { id: undefined, group_id:"babies",      label:"Babies",      sub:"0–24 months",  ratio:4,  color:"#C9929E" },
  { id: undefined, group_id:"toddlers",    label:"Toddlers",    sub:"24–36 months", ratio:5,  color:"#9B7DC0" },
  { id: undefined, group_id:"preschool",   label:"Preschool",   sub:"3–5 years",    ratio:10, color:"#6BA38B" },
  { id: undefined, group_id:"kindergarten",label:"Kindergarten",sub:"4–6 years",    ratio:10, color:"#5B8DB5" },
  { id: undefined, group_id:"oshc",        label:"OSHC",        sub:"5+ years",     ratio:15, color:"#D4A26A" },
];
const purple = "#8B6DAF";
...
function findGroup(ageGroups, id) {
  if (!id) return null;
  const match = ageGroups.find(g => g.group_id === id);
...
function AgeGroupSettingsModal({ onClose, onChanged }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.warn("[AgeGroupSettings] load failed:", e.message);
    }
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
...
// ─── ROOM EDIT MODAL ─────────────────────────────────────────────────────────
function RoomEditModal({ room, ageGroups: ageGroupsProp, onSave, onClose }) {
  const ageGroups = (ageGroupsProp && ageGroupsProp.length > 0) ? ageGroupsProp : DEFAULT_AGE_GROUPS;
...
// ─── AGE WARNING MODAL ───────────────────────────────────────────────────────
function AgeWarningModal({ child, toGroup, complianceIssue, onConfirm, onCancel }) {
  const [comment, setComment] = React.useState("");
  const [err, setErr] = React.useState("");
  const inp2 = { padding:"10px 12px", borderRadius:8, border:"1px solid #DDD", fontSize:13, width:"100%", boxSizing:"border-box", fontFamily:"inherit" };
  const handleConfirm = () => {
    if (!comment.trim()) { setErr("A comment is required to place a child outside their age group."); return; }
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#fff",borderRadius:16,padding:28,maxWidth:500,width:"94%",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
        <div style={{fontSize:36,textAlign:"center",marginBottom:12}}>⚠️</div>
        <h3 style={{margin:"0 0 10px",color:"#B71C1C",textAlign:"center",fontSize:16}}>Age Group Mismatch — Override Required</h3>
        <p style={{margin:"0 0 14px",fontSize:13,color:"#3D3248",lineHeight:1.6,textAlign:"center"}}>
          <strong>{child.first_name} {child.last_name}</strong> is <strong>{ageLabel(child.dob)}</strong>, outside the <strong>{toGroup?.label}</strong> range ({toGroup?.sub}).
        </p>
        {complianceIssue && (
          <div style={{background:"#FFEBEE",borderRadius:10,padding:"12px 16px",marginBottom:14,border:"2px solid #FFCDD2"}}>
            <div style={{fontSize:12,fontWeight:800,color:"#B71C1C",marginBottom:4}}>⚠ NQF COMPLIANCE BREACH</div>
            <div style={{fontSize:12,color:"#B71C1C",lineHeight:1.7}}>{complianceIssue}</div>
            <div style={{fontSize:11,color:"#B71C1C",marginTop:6,fontWeight:600}}>The Centre Manager will be notified and this will be recorded in the audit trail.</div>
          </div>
        )}
        <div style={{background:"#FFF3E0",borderRadius:10,padding:"12px 16px",marginBottom:16,border:"1px solid #FFCC80"}}>
          <div style={{fontSize:12,fontWeight:700,color:"#E65100",marginBottom:6}}>Proceeding will:</div>
          <div style={{fontSize:12,color:"#555",lineHeight:1.8}}>
            • Place {child.first_name} outside their designated age range<br/>
            • Record your override + comment in the child audit trail<br/>
            • Flag on NQF ratio compliance reports
            {complianceIssue && <><br/>• <strong>Notify Centre Manager immediately</strong></>}
          </div>
        </div>
        <div style={{marginBottom:16}}>
          <label style={{fontSize:11,fontWeight:700,color:"#8A7F96",display:"block",marginBottom:6,textTransform:"uppercase"}}>
            Reason for Override <span style={{color:"#B71C1C"}}>* required</span>
          </label>
          <textarea value={comment} onChange={e=>{setComment(e.target.value);setErr("");}} rows={3}
            placeholder="Explain why this child is being placed outside their age group (e.g. sibling arrangement, developmental assessment, temporary)..."
            style={{...inp2,height:"auto",resize:"vertical",lineHeight:1.6}}/>
          {err && <div style={{color:"#B71C1C",fontSize:12,marginTop:4}}>⚠ {err}</div>}
        </div>
        <div style={{display:"flex",gap:10}}>
          ...
          <button onClick={handleConfirm} style={{flex:1,padding:"10px 0",borderRadius:8,border:"none",background:"#B71C1C",color:"#fff",cursor:"pointer",fontWeight:700,fontSize:13}}>Confirm Override</button>
        </div>
      </div>
    </div>
  );
...
// ─── CHILD CHIP ──────────────────────────────────────────────────────────────
function ChildChip({child,group,isDragging,isMoving,fitsGroup,overrideComment,overrideBy,onDragStart,onDragEnd,onRemove}){
  const [showTip,setShowTip]=React.useState(false);
  return(
    <div style={{position:"relative",display:"inline-flex"}}
      onMouseEnter={()=>setShowTip(true)} onMouseLeave={()=>setShowTip(false)}>
      <div draggable={!isMoving} onDragStart={e=>{e.stopPropagation();onDragStart();}} onDragEnd={onDragEnd}
        style={{display:"flex",alignItems:"center",gap:5,padding:"4px 6px 4px 8px",borderRadius:20,
          background:isDragging?lightPurple:!fitsGroup?"#FFEBEE":"#fff",
          border:`1px solid ${isDragging?purple:!fitsGroup?"#FFCDD2":(group?.color||purple)+"50"}`,
          cursor:isDragging?"grabbing":"grab",opacity:isDragging?0.5:isMoving?0.4:1,
          transition:"all 0.15s",userSelect:"none",boxShadow:"0 1px 3px rgba(0,0,0,0.05)"}}>
        <ChildAvatar child={child} size={22} color={!fitsGroup?"#C9828A":group?.color}/>
        <span style={{fontSize:11,fontWeight:600,color:!fitsGroup?"#B71C1C":"#3D3248",maxWidth:68,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{child.first_name}</span>
        {!fitsGroup&&<span style={{fontSize:9,color:"#B71C1C"}}>⚠</span>}
        {isMoving?<span style={{fontSize:10,color:"#8A7F96"}}>…</span>:(
          onRemove&&<button onClick={e=>{e.stopPropagation();onRemove();}} title="Unassign from room"
            style={{width:16,height:16,borderRadius:"50%",border:"none",background:!fitsGroup?"#FFCDD2":"#E5DEFF",color:!fitsGroup?"#B71C1C":"#6B21A8",cursor:"pointer",fontSize:12,lineHeight:"16px",padding:0,flexShrink:0,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
        )}
      </div>
      {showTip&&!fitsGroup&&(
        <div style={{position:"absolute",bottom:"calc(100% + 6px)",left:"50%",transform:"translateX(-50%)",background:"#1C0F2E",color:"#fff",borderRadius:8,padding:"8px 12px",fontSize:11,lineHeight:1.5,maxWidth:240,zIndex:100,pointerEvents:"none",boxShadow:"0 4px 16px rgba(0,0,0,0.3)"}}>
          <div style={{fontWeight:700,marginBottom:3}}>⚠ Outside age range: {ageLabel(child.dob)}</div>
          {overrideComment&&<div style={{opacity:0.85}}>"{overrideComment}"{overrideBy&&<span style={{opacity:0.6}}> — {overrideBy}</span>}</div>}
          {!overrideComment&&<div style={{opacity:0.7,fontStyle:"italic"}}>No override comment recorded</div>}
          <div style={{position:"absolute",bottom:-4,left:"50%",transform:"translateX(-50%)",width:8,height:8,background:"#1C0F2E",rotate:"45deg"}}/>
        </div>
      )}
    </div>
  );
...
export default function RoomsModule() {
  ...
  const load = useCallback(async()=>{
    setLoading(true);
    // Use allSettled so one failing endpoint doesn't blank all three
    const [rmRes, chRes, agRes] = await Promise.allSettled([
      API("/api/rooms"),
      API("/api/children"),
      API("/api/age-groups"),
    ]);
    ...
    if (agRes.status === "fulfilled" && Array.isArray(agRes.value)) {
      setAgeGroups(agRes.value);
    } else {
      console.warn("[RoomsModule] age-groups failed:", agRes.reason?.message);
    }

    if (rmRes.status === "fulfilled" && Array.isArray(rmRes.value)) {
      setRooms(rmRes.value.map(r=>({...r, ageGroup: r.age_group||r.ageGroup})));
    } else {
      console.error("[RoomsModule] rooms failed:", rmRes.reason?.message);
      if(window.showToast) window.showToast("Could not load rooms: " + (rmRes.reason?.message||"unknown"), "error");
    }

    if (chRes.status === "fulfilled" && Array.isArray(chRes.value)) {
      setChildren(chRes.value);
    } else {
      console.warn("[RoomsModule] children failed:", chRes.reason?.message);
    }
  ...
  // Handle deep-link navigation from Settings page
  useEffect(()=>{
    const handler = e => {
      if(e.detail?.action==="add_room") setShowAddRoom(true);
      if(e.detail?.action==="age_group_settings") setShowAgeGroupSettings(true);
    };
  ...
  const saveRoom = async(data) => {
    try {
      if(data.id) {
        ...
        await API("/api/rooms", {method:"POST", body:JSON.stringify({name:data.name, ageGroup:data.ageGroup||"preschool", capacity:data.capacity||20, description:data.description||""})});
      }
      await load();
      if(window.showToast) window.showToast(data.id ? "Room updated ✓" : "Room created ✓");
      setEditRoom(null);
      setShowAddRoom(false);
    } catch(e) {
      console.error("[RoomsModule] saveRoom failed:", e.message);
  ...
  const rightPanelChildren=(()=>{
    const pool=unassigned; // right panel only shows unassigned children; move between rooms by dragging chips on room cards
  ...
  const doMove=async(childId,toRoomId,overrideComment="",overrideBy="")=>{
    if(moving===childId)return;
    setMoving(childId);
    try{
      const body={room_id:toRoomId||null};
      if(overrideComment){body.room_override_comment=overrideComment;body.room_override_by=overrideBy;}
      const result = await API(`/api/children/${childId}`,{method:"PUT",body:JSON.stringify(body)});
      console.log("[doMove] server response:", result);
      // Update local state immediately
      setChildren(prev=>prev.map(c=>c.id===childId?{...c,room_id:toRoomId||null}:c));
      if(window.showToast) window.showToast(toRoomId ? "Child moved ✓" : "Child unassigned ✓");
    }catch(e){
      console.error("[doMove] PUT failed:", e.message);
      if(window.showToast)window.showToast("Move failed: "+e.message,"error");
    }
    setMoving(null);
  };
  ...
  const attemptMove=(childId,toRoomId)=>{
    ...
    if(toGroup&&child?.dob&&!childFitsGroup(child,toGroup)){
      if(!canOverride){if(window.showToast)window.showToast(`${child.first_name} doesn't fit the ${toGroup.label} age range. Director or admin permissions required.`,"error");return;}
      // Check if this creates a compliance issue (ratio breach)
      const otherKids=roomKids.filter(c=>childFitsGroup(c,toGroup));
      const breach=otherKids.length>=(toRoom?.capacity||99)?`This will exceed the capacity for ${toGroup.label} in ${toRoom?.name}.`:null;
      setWarning({child,toRoomId,toGroup,complianceIssue:breach});
      return;
    }
  ...
  const modals = <>
    ...
    {warning && (
      <AgeWarningModal child={warning.child} toGroup={warning.toGroup} complianceIssue={warning.complianceIssue}
        onConfirm={(comment)=>{
          const who=userRole||"manager";
          doMove(warning.child.id,warning.toRoomId,comment,who);
          setWarning(null);
        }}
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <button onClick={async()=>{
            ...
            for(const child of unassigned){
              ...
              const ageM = ageMonths(child.dob);
              if(ageM===null) continue;
              const bestRoom = rooms.find(r=>{
                const g=findGroup(ageGroups,r.ageGroup||r.age_group);
                return g && ageM>=g.min_months && ageM<g.max_months;
              });
              if(bestRoom){
                try{
                  await API(`/api/children/${child.id}`,{method:"PUT",body:JSON.stringify({room_id:bestRoom.id})});
                  moved++;
                }catch(e){}
              }
            }
            if(window.showToast) window.showToast(`${moved} children auto-assigned ✓`);
            load();
          }}
          disabled={unassigned.length===0}
          style={{background:unassigned.length===0?"#E0D8F0":"#E8F5E9",color:unassigned.length===0?"#A89DB5":"#2E7D32",border:`1px solid ${unassigned.length===0?"#D0C8E0":"#A5D6A7"}`,borderRadius:8,padding:"10px 16px",cursor:unassigned.length===0?"default":"pointer",fontWeight:700,fontSize:13}}>
          ✨ Auto-Assign ({unassigned.length})
        </button>
        <button onClick={async()=>{
            // Save all current room assignments to DB in one transaction
            const assignments = children.map(c=>({child_id:c.id,room_id:c.room_id||null}));
            try{
              const r = await API("/api/children/bulk-room-assign",{method:"POST",body:JSON.stringify({assignments})});
              if(window.showToast) window.showToast(`✓ All assignments saved (${r.updated} updated)`);
            }catch(e){
              if(window.showToast) window.showToast("Save failed: "+e.message,"error");
            }
          }} style={{background:"#E3F2FD",color:"#1565C0",border:"1px solid #90CAF9",borderRadius:8,padding:"10px 16px",cursor:"pointer",fontWeight:700,fontSize:13}}>
          💾 Save All
        </button>
        <button onClick={()=>{
          const today=new Date().toLocaleDateString("en-AU",{weekday:"long",day:"numeric",month:"long",year:"numeric"});
          const roomRows=rooms.map(r=>{
            const kids=(childrenByRoom[r.id]||[]);
            const allergyKids=kids.filter(c=>c.allergies&&c.allergies!=="None");
            return `<div style="break-inside:avoid;margin-bottom:16px;border:1px solid #ddd;border-radius:8px;overflow:hidden">
              <div style="background:#3D3248;color:#fff;padding:8px 12px;font-weight:bold">${r.name} — ${kids.length}/${r.capacity||"?"} children</div>
              <div style="padding:8px 12px">
                <div style="display:flex;flex-wrap:wrap;gap:4;margin-bottom:6px">${kids.map(c=>`<span style="background:#f0f0f0;padding:2px 8px;border-radius:12px;font-size:12px">${c.first_name} ${c.last_name}</span>`).join("")||"<em>No children assigned</em>"}</div>
                ${allergyKids.length?`<div style="color:#B71C1C;font-size:11px;font-weight:bold">⚠ Allergies: ${allergyKids.map(c=>c.first_name+" — "+c.allergies).join(" · ")}</div>`:""}
              </div>
            </div>`;
          }).join("");
          const w=window.open("","_blank");
          w.document.write(`<html><head><title>Room Summary</title><style>body{font-family:Arial,sans-serif;font-size:13px;padding:20px}h2{color:#3D3248}@media print{.noprint{display:none}}</style></head><body><h2>Room Summary — ${today}</h2>${roomRows}<div class="noprint" style="margin-top:16px"><button onclick="window.print()" style="padding:8px 20px;background:#3D3248;color:#fff;border:none;border-radius:6px;cursor:pointer">Print</button></div></body></html>`);
          w.document.close();
        }} style={{background:"#F3E8FF",color:"#7C3AED",border:"1px solid #DDD6EE",borderRadius:8,padding:"10px 16px",cursor:"pointer",fontWeight:700,fontSize:13}}>
          🖨 Print Rooms
        </button>
                ...
                <div key={room.id} onDragOver={e=>onDragOver(e,room.id)} onDrop={e=>onDrop(e,room.id)}
                  ...
                  <div style={{padding:"0 12px 14px",minHeight:56}}>
                    {roomKids.length===0?(
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                        {roomKids.map(child=>(<ChildChip key={child.id} child={child} group={group} isDragging={dragging?.childId===child.id} isMoving={moving===child.id} fitsGroup={childFitsGroup(child,group)} onDragStart={()=>onDragStart(child.id,room.id)} onDragEnd={onDragEnd} overrideComment={child.room_override_comment} overrideBy={child.room_override_by} onRemove={()=>doMove(child.id,null)}/>))}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div style={{background:"#fff",borderRadius:14,overflow:"hidden",border:`2px solid ${dragOver==="__unassign__"?purple:"#EDE8F4"}`,transition:"border-color 0.15s"}}>
            <div style={{padding:"14px 16px 10px",borderBottom:"1px solid #EDE8F4",background:selectedGroup?selectedGroup.color+"12":lightPurple}}>
              {selectedRoom?(<>
                ...
                <div style={{fontSize:11,color:"#8A7F96",marginTop:4}}>Showing {unassigned.length} unassigned · Age-appropriate first</div>
              </>):(<>
            ...
            <div style={{padding:10,maxHeight:500,overflowY:"auto"}}>
              {rightPanelChildren.length===0?(
                <div style={{textAlign:"center",padding:"24px 0",color:"#B0AAB9",fontSize:12}}>{unassigned.length===0?"All children assigned 🎉":"No results"}</div>
              ):(
