...
r.get('/families/:id/stories', (req, res) => {
  ...
  const rows = D().prepare(`SELECT ls.*, u.name as educator_name_u FROM learning_stories ls LEFT JOIN users u ON u.id=ls.educator_id WHERE ls.tenant_id=? AND ls.published=1 ORDER BY ls.date DESC LIMIT 50`).all(req.tenantId);
...
r.get('/stories', (req, res) => {
  ...
  let sql = `SELECT ls.*, u.name as educator_name_u FROM learning_stories ls LEFT JOIN users u ON u.id=ls.educator_id WHERE ls.tenant_id=? AND ${periodWhere(period)}`;
...
r.get('/child/:childId/stories', (req, res) => {
  ...
  const rows = D().prepare(`SELECT ls.*, u.name as educator_name_u FROM learning_stories ls LEFT JOIN users u ON u.id=ls.educator_id WHERE ls.tenant_id=? AND ls.published=1 AND ${periodWhere(period)} ORDER BY ls.date DESC LIMIT 50`).all(req.tenantId);
