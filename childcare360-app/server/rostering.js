...
r.get('/periods', (req, res) => {
  const periods = D().prepare(`SELECT rp.*, 
...
r.get('/periods/:id', (req, res) => {
  ...
  const entries = D().prepare(`SELECT re.*, e.first_name || ' ' || e.last_name as educator_name,
    e.qualification, e.hourly_rate_cents, e.reliability_score, e.employment_type,
...
r.put('/periods/:id/publish', (req, res) => {
  D().prepare("UPDATE roster_periods SET status='published', updated_at=datetime('now') WHERE id=?").run(req.params.id);
  res.json({ ok: true });
});
...
r.post('/generate', (req, res) => {
  ...
  const educators = db.prepare('SELECT * FROM educators WHERE tenant_id = ? AND status = \'active\' ORDER BY reliability_score DESC').all(req.tenantId);
  const availability = {};
  educators.forEach(e => {
...
r.post('/sick-cover-optimise', (req, res) => {
  try {
    ...
    const casuals = db.prepare(`
      ...
      WHERE e.tenant_id=? AND e.employment_type='casual' AND e.status='active'
        AND e.id != ?
        AND e.id NOT IN (SELECT educator_id FROM roster_entries WHERE date=? AND tenant_id=?)
