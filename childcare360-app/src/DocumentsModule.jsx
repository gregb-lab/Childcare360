...
export default function DocumentsModule() {
  ...
  const denyDoc = async (id) => {
    await API(`/api/documents/${id}/deny`, { method: "PUT", body: JSON.stringify({ reason: "Denied by centre manager" }) });
    load();
  };
  ...
  return (
    <tr style={{ borderBottom: "1px solid #F0EBF8" }}>
      ...
      <td style={{ padding: "10px 12px", color: "#8A7F96" }}>
        {doc.created_at ? new Date(doc.created_at).toLocaleDateString("en-AU") : "—"}
      </td>
      <td style={{ padding: "10px 12px" }}>
        {doc.expiry_date ? (
