        ...
        <div style={{ ...card, marginBottom: 16, display: "flex", gap: 20, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: 16, fontSize: 13, color: "#8A7F96", marginBottom: 8 }}>
              ...
              <span>📅 Enrolled {new Date(child.enrolledDate).toLocaleDateString("en-AU", { month: "short", year: "numeric" })}</span>
                ...
                {childObs.slice(0, 3).map(o => (
                  <div key={o.id} style={{ background: "#F8F5F1", borderRadius: 8, padding: 10, marginBottom: 6, borderLeft: `3px solid ${EYLF_OUTCOMES.find(e => e.subOutcomes.some(s => (o.eylfOutcomes || []).includes(s.id)))?.color || "#A89DB5"}` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 10, color: "#A89DB5" }}>{new Date(o.timestamp).toLocaleDateString("en-AU")}</span>
                    </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <span style={{ fontSize: 13, color: "#8A7F96" }}>
                {new Date(plan.date).toLocaleDateString("en-AU", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
            ...
            {pastPlans.slice(0, 6).map(p => (
              <div key={p.id} onClick={() => setViewPlan(p)} style={{ ...cardSm, cursor: "pointer", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  ...
                  <span style={{ fontSize: 12, color: "#A89DB5", marginLeft: 12 }}>{new Date(p.date).toLocaleDateString("en-AU", { weekday: "short", day: "numeric", month: "short" })}</span>
                </div>
...
export function ObservationsView({ children, rooms, observations, setObservations }) {
  ...
  const attachMedia = () => {
    ...
    setFormMedia(prev => [...prev, { id: `media_${Date.now()}`, type, name: `${type} — ${new Date().toLocaleTimeString("en-AU")}` }]);
  };
      ...
      {filteredObs.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filteredObs.map(obs => {
            ...
            const time = new Date(obs.timestamp).toLocaleTimeString("en-AU", { hour: "2-digit", minute: "2-digit" });
