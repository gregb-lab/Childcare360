      ...
      <div style={{ background: `linear-gradient(135deg,${purple},#6B4FA0)`, color: "#fff", padding: "16px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div>
            ...
            <p style={{ margin: "2px 0 0", fontSize: 12, opacity: 0.8 }}>Welcome back · {new Date().toLocaleDateString("en-AU",{weekday:"long",day:"numeric",month:"long"})}</p>
          </div>
          {children.length > 1 && (
...
function ParentMessages({ messages, onRefresh }) {
  ...
  const send = async () => {
    ...
    await API("/api/parent/messages", { method: "POST", body: { subject, body: newMsg } });
    ...
    <div>
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
          <div><label style={lbl}>Start Date</label><input type="date" style={inp} value={f.start_date} onChange={e => setF(p => ({ ...p, start_date: e.target.value }))} /></div>
          <div><label style={lbl}>End Date (leave empty for 1 day)</label><input type="date" style={inp} value={f.end_date} onChange={e => setF(p => ({ ...p, end_date: e.target.value }))} /></div>
