...
export default function DailyUpdatesModule() {
  ...
  const loadUpdates = useCallback(async (childId) => {
    try {
      const r = await API(`/api/children/${childId}/daily-updates?date=${todayStr()}`);
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div>
            ...
            <p style={{ margin: "2px 0 0", color: "#8A7F96", fontSize: 12 }}>
              {new Date().toLocaleDateString("en-AU",{weekday:"long",day:"numeric",month:"long"})} · Real-time child activity logging · Visible to parents in portal
            </p>
          </div>
...
function saveUpdate(childId, data) {
  return API(`/api/children/${childId}/daily-updates`, { method: "POST", body: data });
}
...
function SleepForm({ child, onSaved }) {
  ...
  const endSleep = async () => {
    const endTime = now();
    if (sleepEntry) await API(`/api/children/${child.id}/daily-updates/${sleepEntry.id}`, { method: "PUT", body: { end_time: endTime, status: "awake" } });
    setSleeping(false);
    onSaved(child.id);
  };
...
function IncidentForm({ child, onSaved }) {
  ...
  return (
    <div style={{ ...card, background: "#FFEBEE", border: "2px solid #FFCDD2", marginBottom: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 8 }}>
        <div><label style={lbl}>Date</label><input type="date" style={inp} value={f.date} onChange={e => setF(p => ({ ...p, date: e.target.value }))} /></div>
        <div><label style={lbl}>Time</label><input type="time" style={inp} value={f.time} onChange={e => setF(p => ({ ...p, time: e.target.value }))} /></div>
