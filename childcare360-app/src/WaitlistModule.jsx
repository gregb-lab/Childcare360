...
export default function WaitlistModule() {
  ...
  const addToWaitlist = async () => {
    ...
    await API("/api/waitlist", { method: "POST", body: form }).catch(()=>{});
    setShowAdd(false);
  ...
  const removeFromWaitlist = async (id) => {
    ...
    await API(`/api/waitlist/${id}`, { method: "DELETE" }).catch(()=>{});
    load();
  };
  ...
  const approveSuggestion = async (childId, roomId, isNew = false) => {
    ...
    if (isNew) {
      ...
      await API(`/api/waitlist/${childId}/convert`, { method: "POST", body: { room_id: roomId } }).catch(()=>{});
    } else {
      await API(`/api/children/${childId}`, { method: "PUT", body: { room_id: roomId } }).catch(()=>{});
    }
    load();
  };
