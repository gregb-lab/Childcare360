...
r.get('/my', requireAuth, (req, res) => {
  ...
  const _wb1 = '-' + days + ' days'; const rows = D().prepare('SELECT * FROM staff_wellbeing WHERE user_id=? AND date >= date(\'now\',?,\'localtime\') ORDER BY date DESC').all(req.userId, _wb1);
  res.json(rows);
});
...
r.get('/history', requireAuth, requireTenant, requireRole('admin', 'director'), (req, res) => {
  initWellbeing();
  const days = parseInt(req.query.days) || 30;
  const _wb2 = '-' + days + ' days'; const rows = D().prepare('SELECT sw.*, u.name FROM staff_wellbeing sw LEFT JOIN users u ON u.id=sw.user_id WHERE sw.tenant_id=? AND sw.date >= date(\'now\',?,\'localtime\') ORDER BY sw.date DESC').all(req.tenantId, _wb2);
  // Mask anonymous
