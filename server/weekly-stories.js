...
function gatherChildData(tid, child_id, from, to, maxItems = 30) {
  ...
  const photos = [
    ...
    ...((() => { const _wc = '%' + child_id + '%'; return D().prepare('SELECT sp.url, ls.date FROM story_photos sp JOIN learning_stories ls ON ls.id=sp.story_id WHERE sp.tenant_id=? AND ls.date BETWEEN ? AND ? AND instr(ls.child_ids, ?) > 0 LIMIT 15').all(tid, from, to, _wc); })()),
