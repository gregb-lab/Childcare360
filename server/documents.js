...
function analyseDocument(docId, tenantId) {
  try {
    ...
    D().prepare("UPDATE child_documents SET ai_status='processing' WHERE id=?").run(docId);

    const child = D().prepare('SELECT * FROM children WHERE id=?').get(doc.child_id);
    ...
    D().prepare("UPDATE child_documents SET ai_extracted=?,ai_status='complete' WHERE id=?").run(JSON.stringify(extracted), docId);
    console.log(`  ✓ AI complete: ${doc.file_name}`);
  } catch (err) {
    console.error('  ✗ AI error:', err.message);
    D().prepare("UPDATE child_documents SET ai_status='error',ai_extracted=? WHERE id=?").run(JSON.stringify({error:err.message}), docId);
  }
}
