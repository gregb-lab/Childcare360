...
function parentChildIds(userId) {
  ...
  const membership = D().prepare("SELECT role FROM tenant_members WHERE user_id=? AND role IN ('admin','director','educator','manager') LIMIT 1").get(userId);
  if (membership) {
    // Staff: return all active children for their tenant(s) as preview
    const tenantIds = D().prepare("SELECT DISTINCT tenant_id FROM tenant_members WHERE user_id=? AND role IN ('admin','director','educator','manager')").all(userId).map(r=>r.tenant_id);
    if (tenantIds.length) {
