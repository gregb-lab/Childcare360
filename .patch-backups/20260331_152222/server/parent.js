...
// FIX (sql-injection-risk): previously interpolated SQLite date() expressions
// directly into prepared statement strings. Now computed in JS, bound via ?.
...
r.get('/learning/:childId', (req, res) => {
  try {
    ...
    // FIX (sql-injection-risk + sql-missing-tenant-id)
...
r.get('/learning/:childId/weekly-report', (req, res) => {
  try {
    const childIds = parentChildIds(req.userId);
    if (!childIds.includes(req.params.childId)) return res.status(403).json({ error: 'Not authorised' });
    // FIX (sql-missing-tenant-id): weekly_reports + learning_stories
    const tenantId = childTenantId(req.params.childId);
    const rep = D().prepare(`
